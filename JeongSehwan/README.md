...
### Free인데 offline=1이 존재하는 “예외 구간”에 대한 가설

일반적으로 **Free는 오프라인 청취가 불가능**하므로, `Free & offline=1` 조합은 현실 규칙과 맞지 않는 값(예외 케이스)이다.

그럼에도 해당 집단에서 이탈률이 **26.26%**로 관측되는 이유는 아래의 가설로 설명한다.

### “무료 체험/프로모션 후 만료 이탈”

 사용자는 과거 일정 기간 **Premium 체험(예:무료 체험 기간)** 을 통해 오프라인 기능을 사용했으나,
 **체험 종료 후 현재 상태가 Free로 전환**되면서 구독 유형은 Free로 기록되고,
 이후 **만료 시점/전환 거부**로 이탈이 발생했을 수 있다.

이 경우 `offline_listening`은 “Free에서도 가능한 기능”이 아니라 데이터 로그상의 과거 **Premium 경험(이력)의 흔적**으로 해석된다.
