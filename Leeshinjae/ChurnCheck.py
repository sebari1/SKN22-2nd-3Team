...
if ml_prob is not None and dl_prob is not None:
    ...
    with cols[0]:
        ...
        st.metric("임계값", f"{best_ml_threshold*100:.1f}%")
        st.metric("정확도", f"{metrics.get(best_ml_name, {}).get('Accuracy', 0)*100:.1f}%")
        st.metric("F1-Score", f"{metrics.get(best_ml_name, {}).get('F1-Score', 0):.3f}")
    ...
    with cols[1]:
        ...
        st.metric("임계값", f"{dl_threshold*100:.1f}%")
        st.metric("정확도", f"{metrics.get('Deep Learning (DNN)', {}).get('Accuracy', 0)*100:.1f}%")
        st.metric("F1-Score", f"{metrics.get('Deep Learning (DNN)', {}).get('F1-Score', 0):.3f}")
    ...
    with col_avg1:
        st.metric("평균 이탈 확률", f"{avg_prob*100:.1f}%")
    with col_avg2:
        st.metric("위험도", avg_risk_text.replace("🚨 ", "").replace("⚠️ ", "").replace("✅ ", ""))
    with col_avg3:
